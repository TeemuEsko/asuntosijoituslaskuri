...
test("vanhan parserin HTML-fixture palauttaa kaikki peruskentät", () => {
  ...
  assert.deepEqual(fields, { listingId: "123456", debtFreePrice: 89_000, sellingPrice: 75_500, maintenanceFee: 245.5, financingFee: 112.3, size: 32.5, buildYear: 1987, buildingType: "apartment", heatingType: "district", landType: "own" });
});
...
test("suora velkaosuus voittaa hintojen erotuksen", () => {
  ...
  assert.equal(direct.fields.debtShare, 15_000);
  const fallback = parseLegacyListingHtml("Myyntihinta 144 000 € Velaton hinta 159 000 €");
  assert.equal(fallback.fields.debtShare, undefined);
  ...
  assert.equal(fallbackFinding?.originalLabel, "Päätelty hinnoista");
});
