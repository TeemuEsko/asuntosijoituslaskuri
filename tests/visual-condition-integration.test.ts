...
import { visualConditionReportDisclaimer, visualConditionUiDisclaimer } from "../src/core/visual-condition/types.ts";
...
test("raportti sisältää vahvistetut kuvahavainnot, lähdehistorian ja raportin vastuuvapautuksen ilman kuvatiedostoja", () => {
  ...
  assert.equal(report.visualCondition?.disclaimer, visualConditionReportDisclaimer(visual.source)); assert.equal(report.visualCondition?.analysis.observations.length, 2); assert.match(JSON.stringify(report), /sourceHistory/); assert.doesNotMatch(JSON.stringify(report), /data:image|previewUrl|image\/jpeg/);
});
...
test("käyttöliittymä sisältää valinnan, rajaukset, etenemisen, muokkauksen ja virhetilat", async () => {
  ...
  assert.ok(types.includes("Arvio ei korvaa kuntotarkastusta")); assert.match(component, /visualConditionUiDisclaimer/); assert.match(visualConditionUiDisclaimer("listing_session"), /myynti-ilmoituksen valokuviin/);
