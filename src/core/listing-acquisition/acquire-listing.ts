...
export async function acquireListing(urlInput: string, options: { browserProvider?: ListingBrowserProvider | null; forceRefresh?: boolean; fetchImpl?: typeof fetch; skipDnsCheck?: boolean } = {}): Promise<ListingAcquisitionResult> {
  ...
  if (reason) {
    ...
    if (provider) {
      ...
      if (browser.ok) { finalResult = parseListingText(parserInputFromHtml(browser.html, `${browser.visibleText}\n${browser.contexts.map((context) => context.excerpt).join("\n")}`), validation.source, extractStructuredValues(browser.html)); method = "browser"; browserDiagnostics = browser.diagnostics; contentContexts = browser.contexts; }
