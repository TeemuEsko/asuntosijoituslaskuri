...
export type InvestmentAnalysisResult = InvestmentOverallScoreData & {
  preliminary: boolean; grossRentalYield?: number; netRentalYield?: number; effectiveAnnualRent?: number; cashFlowBeforeBankLoan?: number; cashFlowAfterBankLoan?: number; monthlyBankLoanPayment?: number; monthlyBankLoanInterest?: number; monthlyBankLoanPrincipal?: number; bankLoanAmount?: number; equity?: number;
};
...
export function calculateInvestmentAnalysis(input: InvestmentAnalysisInput): InvestmentAnalysisResult {
  ...
  const leverage = price && valid(input.bankLoanAmount) ? input.bankLoanAmount / price : undefined;
  ...
  const demandComponent = clampInvestmentScore(20 + (input.rentalDemand ?? 3) * 15 - vacancy.vacancyMonths * 4);
