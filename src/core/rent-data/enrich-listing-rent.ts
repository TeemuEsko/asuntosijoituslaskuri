...
export type RentEnrichmentOptions = { fetcher?: typeof fetch; logger?: Pick<Console, "error"> };
...
export async function addAutomaticRentEstimate(result: ListingParseResult, options: RentEnrichmentOptions = {}): Promise<ListingParseResult> {
  ...
  const listingRent = accepted("currentRentMonthly"); const areaSqm = accepted("areaSqm"); const roomDescription = accepted("roomDescription"); const city = accepted("city"); const district = accepted("district");
  const location = resolveMunicipalityName({ city: typeof city === "string" ? city : null, district: typeof district === "string" ? district : null });
  const benchmarkResult = await fetchStatisticsFinlandRentBenchmark({ municipality: location.municipality, roomDescription: typeof roomDescription === "string" ? roomDescription : null, areaSqm: typeof areaSqm === "number" ? areaSqm : null, fetcher: options.fetcher });
