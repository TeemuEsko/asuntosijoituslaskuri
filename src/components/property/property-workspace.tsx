...
import { AssumptionsCard, type AssumptionFieldKey, type AssumptionValues } from "./assumptions-card";
...
export function PropertyWorkspace({ importedData = {}, title, onRequestEvaluation }: { importedData?: ImportedPropertyData; title?: string; onRequestEvaluation?: () => void }) {
  const initialRentEstimate: RentEstimate = importedData.rentEstimate ?? (typeof importedData.currentRentMonthly === "number" ? { effectiveMonthlyRent: importedData.currentRentMonthly, source: "listing", sourceName: "Myynti-ilmoitus", confidence: "high", userOverridden: false, resolutionStatus: "resolved" } : { effectiveMonthlyRent: null, source: "unknown", confidence: "unknown", userOverridden: false, resolutionStatus: "unavailable" });
  const initialAutomaticRent: RentEstimate = initialRentEstimate.userOverridden && initialRentEstimate.benchmark ? initialRentEstimate.benchmark : initialRentEstimate;
  const [data, setData] = useState(importedData);
  const [purchase, setPurchase] = useState<Record<PurchaseFieldKey, number>>(() => purchaseFromImport(importedData));
  const [statuses, setStatuses] = useState<Record<PurchaseFieldKey, FieldStatus>>({ debtFreePrice: importedData.debtFreePrice === undefined ? "missing" : "parser", salePrice: importedData.salePrice === undefined ? "missing" : "parser", companyLoanShare: importedData.companyLoanShare === undefined ? "missing" : "parser", financingFeeMonthly: importedData.financingFeeMonthly === undefined ? "missing" : "parser", renovationReserve: importedData.visualCondition?.confirmationStatus === "confirmed" ? importedData.visualCondition.renovationReserveSource === "user" ? "user" : "derived" : "default" });
  ...
  const bankLoanAmount = calculateBankLoanAmount(purchase.salePrice, assumptions.equity);
  ...
  useEffect(() => { try { window.sessionStorage.setItem(ANALYSIS_DRAFT_KEY, JSON.stringify({ ...data, currentRentMonthly: rentEstimate.effectiveMonthlyRent ?? undefined, rentEstimate })); } catch { /* Analyysi toimii myös ilman istuntotallennusta. */ } }, [data, rentEstimate]);

  function updatePurchase(key: PurchaseFieldKey, value: number) { setPurchase((current) => ["debtFreePrice", "salePrice", "companyLoanShare"].includes(key) ? { ...current, ...synchronizePrices(current, key as "debtFreePrice" | "salePrice" | "companyLoanShare", value, lastEditedPriceField) } : { ...current, [key]: value }); if (key === "debtFreePrice" || key === "salePrice") setLastEditedPriceField(key); if (key === "renovationReserve") setRenovationReserveUserEdited(true); setStatuses((current) => ({ ...current, [key]: "user", ...(key === "debtFreePrice" ? { salePrice: "derived" as const } : key === "salePrice" ? { debtFreePrice: "derived" as const } : key === "companyLoanShare" ? { [lastEditedPriceField === "debtFreePrice" ? "salePrice" : "debtFreePrice"]: "derived" as const } : {}) })); }
  function updateAssumption<K extends AssumptionFieldKey>(key: K, value: AssumptionValues[K]) { setAssumptions((current) => key === "equity" ? { ...current, ...userEquityAssumption(Number(value)) } : { ...current, [key]: value }); }
  function resetEquity() { setAssumptions((current) => ({ ...current, ...defaultEquityAssumption() })); }
  function overrideRent(value: number) { setRentEstimate({ effectiveMonthlyRent: value, source: "user", sourceName: "Käyttäjän määrittämä vuokra", confidence: "high", userOverridden: true, previousAutomaticEstimate: automaticRentEstimate.effectiveMonthlyRent, benchmark: automaticRentEstimate.benchmark ?? (["statistics_finland", "fallback"].includes(automaticRentEstimate.source) ? automaticRentEstimate : null), resolutionStatus: "resolved", attemptedSources: ["user"] }); updateAssumption("monthlyRent", value); setAnalysisUpdating(true); window.setTimeout(() => setAnalysisUpdating(false), 500); }
  function restoreRent() { setRentEstimate(automaticRentEstimate); updateAssumption("monthlyRent", automaticRentEstimate.effectiveMonthlyRent ?? 0); }
  function addDocument(kind: RepairDocumentKind) { setData((current) => ({ ...current, documentKinds: [...new Set([...(current.documentKinds ?? []), kind])] })); }
  function updateVisualCondition(visualCondition: VisualConditionAnalysis) { const visualUserEdited = visualCondition.renovationReserveSource === "user"; setData((current) => ({ ...current, visualCondition })); if (visualUserEdited) setRenovationReserveUserEdited(true); if (visualCondition.confirmationStatus === "confirmed" && (!renovationReserveUserEdited || visualUserEdited)) { setPurchase((current) => ({ ...current, renovationReserve: visualCondition.estimatedRenovationCostRange?.recommendedReserve ?? current.renovationReserve })); setStatuses((current) => ({ ...current, renovationReserve: visualUserEdited ? "user" : "derived" })); } }
  function updateImportedField(field: NormalizedFieldKey, value: number | string | undefined) { setData((current) => { const next = { ...current }; if (value === undefined) delete next[field]; else next[field] = value; return next; }); if (["debtFreePrice", "salePrice", "companyLoanShare", "financingFeeMonthly"].includes(field)) setPurchase((current) => ({ ...current, [field]: typeof value === "number" ? value : 0 })); if (field === "maintenanceFeeMonthly" && typeof value === "number") updateAssumption("maintenanceFeeMonthly", value); if (field === "currentRentMonthly" && typeof value === "number") { const next: RentEstimate = { effectiveMonthlyRent: value, source: "user", sourceName: "Käyttäjän määrittämä vuokra", confidence: "high", userOverridden: true, previousAutomaticEstimate: automaticRentEstimate.effectiveMonthlyRent, benchmark: automaticRentEstimate.benchmark, resolutionStatus: "resolved", attemptedSources: ["user"] }; setRentEstimate(next); updateAssumption("monthlyRent", value); } }
