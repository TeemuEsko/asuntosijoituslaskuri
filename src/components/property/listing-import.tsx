...
import { housingCompanyLoanStatus, resolveHousingCompanyLoan } from "@/core/analysis/housing-company-loan";
...
export function ListingImport({
  ...
  async function searchListing(forceRefresh: boolean | unknown = false) {
    ...
    try {
      ...
      if (payload.preparation?.nextStep === "analysis") {
        ...
        onComplete({ ...parsedValues, rentEstimate: payload.rentEstimate, renovations: payload.renovations, housingCompanyRenovations: payload.housingCompanyRenovations, housingCompanyLoan: payload.housingCompanyLoan, documentKinds: ["listing"], importReview: payload, analysisReliability: rentAwareReliability(payload, parsedValues), listingImageAnalysis: payload.listingImageAnalysis, visualCondition: payload.visualCondition });
      } else {
        setResult(payload);
  ...
  function createProperty() {
    const importedValues: ImportedPropertyData = {
      ...
      housingCompanyLoan: result?.housingCompanyLoan,
  ...
  function finishAutomaticAnalysis() {
    ...
    try {
      ...
      const detectedDebt = debtWasEdited ? debtShareStatus(combined) : housingCompanyLoanStatus(result.housingCompanyLoan);
      ...
      const userDebtOverride = typeof userValues.companyLoanShare === "number" || debtChoice !== null;
  ...
  if (result && typeof window !== "undefined") {
    ...
    const detectedDebt = housingCompanyLoanStatus(result.housingCompanyLoan);
