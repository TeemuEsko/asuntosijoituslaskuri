...
function SelectField<T extends string>({ label, value, displayValue, onChange, children }: { label: string; value: T; displayValue: string; onChange: (value: T) => void; children: ReactNode }) {
  return <div className="min-w-0 space-y-2"><div className="flex min-h-10 min-w-0 flex-wrap items-start justify-between gap-x-3 gap-y-1"><Label className="min-w-0 flex-1 whitespace-normal leading-5">{label}</Label><SourceBadge status="user" /></div><Select value={value} onValueChange={(nextValue) => onChange(nextValue as T)}><SelectTrigger className="h-11 w-full min-w-0"><SelectValue>{displayValue}</SelectValue></SelectTrigger><SelectContent>{children}</SelectContent></Select></div>;
}
