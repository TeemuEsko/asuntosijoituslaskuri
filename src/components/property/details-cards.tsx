...
function DetailGrid({ rows }: { rows: ReadonlyArray<readonly [string, string]> }) {
  return <dl className="grid grid-cols-2 gap-x-5 gap-y-4">{rows.map(([label, value]) => <div key={label} className="border-b pb-3"><dt className="text-xs text-muted-foreground">{label}</dt><dd className="mt-1 font-medium">{value}</dd></div>)}</dl>;
}
