...
export function WorkspaceSidebar() {
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-18 min-w-0 border-r bg-sidebar min-[1100px]:flex min-[1100px]:flex-col min-[1600px]:w-60">
      <div className="flex h-20 min-w-0 items-center border-b px-3 min-[1600px]:px-5"><Link href="/" aria-label="Siirry etusivulle" onClick={() => window.dispatchEvent(new Event("property-home"))} className="flex min-w-0 cursor-pointer items-center justify-center gap-2 rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-success focus-visible:ring-offset-2 min-[1600px]:justify-start"><div className="grid size-8 shrink-0 place-items-center rounded-lg bg-success text-white" title={brandName}><Building2 className="size-4" /></div><span className="hidden min-w-0 truncate text-sm font-semibold min-[1600px]:inline">{brandName}</span></Link><Badge variant="outline" className="ml-auto hidden shrink-0 px-1.5 text-[10px] min-[1600px]:inline-flex">Ennakkoversio</Badge></div>
